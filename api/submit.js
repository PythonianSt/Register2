  ...
  try {
    ...
    const value = await mutate(async rows => {
      ...
      const record = {
        ...
        patient_status:['คนไข้ใหม่','เคยใช้บริการ','ไม่แน่ใจ'].includes(clean(b.patient_status)) ? clean(b.patient_status) : '',
        student_or_staff_id:clean(b.student_or_staff_id,50), email:clean(b.email,120), national_id:clean(b.national_id,50),
        title_th:clean(b.title_th,50), title_other:clean(b.title_other,100),
